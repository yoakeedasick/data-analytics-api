...
def generate_pdf_report(file_name: str, row_count: int, col_count: int, result_json: dict) -> io.BytesIO:
    ...
    story.append(Paragraph(f"Generated on {datetime.now().strftime('%b %d, %Y %I:%M %p')} | Dataset: {file_name}", subtitle_style))
    ...
    integrity_pct = round(((total_cells - total_missing) / total_cells) * 100, 2) if total_cells > 0 else 100.0

    overview_data = [
        [Paragraph("Metadata Field", table_header_style), Paragraph("Value", table_header_style)],
        [Paragraph("File Name", table_cell_style), Paragraph(file_name, table_cell_style)],
        [Paragraph("Total Records (Rows)", table_cell_style), Paragraph(f"{row_count:,}", table_cell_style)],
        [Paragraph("Total Features (Columns)", table_cell_style), Paragraph(f"{col_count}", table_cell_style)],
        [Paragraph("Data Integrity (Complete Cells)", table_cell_style), Paragraph(f"{integrity_pct}%", table_cell_style)],
        [Paragraph("Total Missing Values", table_cell_style), Paragraph(f"{total_missing:,}", table_cell_style)]
    ]
    ...
    story.append(Paragraph("Column Specifications & Data Quality", heading_style))
    col_specs_data = [
        [
    ...
    for col in column_names:
        ...
        col_specs_data.append([
            ...
            Paragraph(f"{m_count:,}" if m_count > 0 else "0", table_cell_style),
    ...
    if describe_data:
        story.append(Paragraph("Descriptive Statistics (Numeric Features)", heading_style))
        stats_list = ["count", "mean", "std", "min", "50%", "max"]
        
        describe_headers = [Paragraph("Feature / Column", table_header_style)]
        for s in stats_list:
            describe_headers.append(Paragraph(s.upper() if s != "50%" else "MEDIAN", table_header_style))
    ...
    if correlation:
        ...
        t_corr = Table(corr_table_data, colWidths=[first_col_width] + [rem_width] * num_corr_cols)
    ...
    def add_footer(canvas, doc):
        ...
        canvas.drawString(40, 25, "Dalytics Platform — Automated Statistical Analysis Report")
